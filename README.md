...
Build and run the whole stack (Postgres + backend + frontend):
...
By default the backend will listen on port `8000` and the frontend will be served on `5173` (mapped to nginx port `80` in the container).
...
Notes:
- The compose file uses `ankane/pgvector` image which comes with the `vector` extension preinstalled.
- Mounts: `uploads/` and `logs/` are mounted into the backend service so files are persisted locally.
- For production, customize `GEMINI_API_KEY` and consider using a secrets manager.
